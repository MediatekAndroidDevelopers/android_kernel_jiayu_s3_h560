	...
	do {
		...
		if (prSwRfb) {
			...
			do {
				...
				if (!kalIsWakeupByWlan(prAdapter)/* || !pvHeader*/)
					break;

				switch (prSwRfb->ucPacketType) {
				case HIF_RX_PKT_TYPE_DATA:
					if (pvHeader == NULL) {
						DBGLOG(INIT, INFO, ("RX_PKT_TYPE_RX_DATA pvHeader is NULL\n"));
						break;
					}
				...
				case HIF_RX_PKT_TYPE_MANAGEMENT:
					if (pvHeader == NULL) {
						DBGLOG(INIT, INFO, ("HIF_RX_PKT_TYPE_MANAGEMENT pvHeader is NULL\n"));
						break;
					}
