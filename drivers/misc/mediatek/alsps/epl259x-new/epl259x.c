...
#define MT6580
#if defined(MT6575) || defined(MT6571) || defined(MT6589) || defined(MT6582) || defined(MT6580) || defined(MT6592) || defined(MT6735) || defined(MT6752) || defined(MT6753)
#include <mach/devs.h>
...
#if defined(MT6582) ||defined(MT6580) || defined(MT6592) || defined(MT6735) || defined(MT6752) || defined(MT6753)
...
/*----------------------------------------------------------------------------*/
int epl219x_hw8k_init_device(struct i2c_client *client)
{
    APS_LOG("epl219x_hw8k_init_device.........\r\n");
...
void epl_sensor_reset_dynk_thd(u8 last_status, u8 now_status)
{
    if(last_status==0 && now_status==1)
    {
	    ...
	    APS_LOG("[%s]:restart dynk thre_l = %u, thre_h = %u\n", __func__, dynk_thd_low, dynk_thd_high);
#endif
        eint_flag = false;
...
int epl_sensor_setup_eint(struct i2c_client *client)
{
	...
	mt_eint_registration(CUST_EINT_ALS_NUM, 8, epl_sensor_eint_func, 0);
...
static int epl_sensor_init_client(struct i2c_client *client)
{
    ...
    {
#if defined(MT6582) ||defined(MT6580) || defined(MT6592) || defined(MT6752) || defined(MT6735) || defined(MT6753)
        mt_eint_mask(CUST_EINT_ALS_NUM);
#else
        mt65xx_eint_mask(CUST_EINT_ALS_NUM);
#endif
    ...
    if((err = epl219x_hw8k_init_device(client)) != 0)
    {
        APS_ERR("init dev: %d\n", err);
        return err;
    }
...
static int ps_get_data(int* value, int* status)
{
    ...
    *status = SENSOR_STATUS_ACCURACY_MEDIUM;
    ...
    APS_LOG("[%s]:raw=%d *value = %d\n", __func__, epl_sensor.ps.compare_low, *value);
