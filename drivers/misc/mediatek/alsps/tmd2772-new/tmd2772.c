...
#define TMD2772_SENSOR_ID1 0x30
#define TMD2772_SENSOR_ID2 0x39
...
static struct i2c_board_info __initdata i2c_TMD2772={ I2C_BOARD_INFO("TMD2772", 0x40)};
/*----------------------------------------------------------------------------*/
...
static int TMD2772_get_sensor_id(struct i2c_client *client)
{
  u8 alsps_id[2];
  alsps_id[0] = 0x92;
  struct TMD2772_priv *obj = i2c_get_clientdata(client);
  ...
  res = TMD2772_i2c_master_operate(client, alsps_id, 0x101, I2C_FLAG_READ);
  APS_LOG("alsps_id = 0x%x ", alsps_id[0]);

  if ((alsps_id[0] != TMD2772_SENSOR_ID1)&&(alsps_id[0] != TMD2772_SENSOR_ID2)) {
      alsps_id[0] = 0xFF;
      return -1;
  }
  else
...
static int TMD2772_init_client(struct i2c_client *client)
{
    ...
    res = TMD2772_get_sensor_id(client);
    if(res < 0)
    {
            APS_LOG(" TMD2772_init_client get sensor id err!\n");
            goto EXIT_ERR;
    }
...
static ssize_t TMD2772_show_ps(struct device_driver *ddri, char *buf)
{
    ...
    if(res = TMD2772_read_ps(TMD2772_obj->client, &data))
    {
...
static ssize_t TMD2772_show_psthd(struct device_driver *ddri, char *buf)
{
    int idx;
    if(!TMD2772_obj)
    {
    ...
    return snprintf(buf, PAGE_SIZE, "%d\n", TMD2772_obj->ps_thd_val_high);
}

static ssize_t TMD2772_store_psthd(struct device_driver *ddri, char *buf, size_t count)
{
    int  thres;
    if(!TMD2772_obj)
    {
    ...
    if(1 == sscanf(buf, "%d",&thres))
    {
        ...
        atomic_set(&TMD2772_obj->ps_thd_val_high, thres);
        atomic_set(&TMD2772_obj->ps_thd_val_low, (thres - 80));
    }
    else
    {
...
static DRIVER_ATTR(psthd,   S_IWUSR | S_IRUGO, TMD2772_show_psthd, TMD2772_store_psthd);
/*----------------------------------------------------------------------------*/

static struct device_attribute *TMD2772_attr_list[] = {
        &driver_attr_ps,
        &driver_attr_psthd,
};
