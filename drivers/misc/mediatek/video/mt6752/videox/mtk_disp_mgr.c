...
static DISP_MODE select_session_mode(disp_session_config *session_info)
{
	...
	if (session_info->user == SESSION_USER_GUIEXT) {
#ifdef CONFIG_FOR_SOURCE_PQ
        extern atomic_t isMiraPath;
        #ifdef MTK_DISP_IDLE_LP
        extern void _disp_primary_path_exit_idle(const char*, unsigned int);
        #endif
		/*miravison */
		...
		if (session_info->mode == DISP_SESSION_DECOUPLE_MODE) {
            ...
            atomic_set(&isMiraPath, 1);
            #ifdef MTK_DISP_IDLE_LP
            _disp_primary_path_exit_idle("set_session_mode", 1);
            #endif
