...
{
    pr_info("[bq24196_shutdown] driver shutdown \n");
...
kal_uint32 bq24196_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    pr_info("[bq24196_read_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg);

    bq24196_reg &= (MASK << SHIFT);
    *val = (bq24196_reg >> SHIFT);
    pr_info("[bq24196_read_interface] Val=0x%x\n", *val);
...
kal_uint32 bq24196_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = bq24196_read_byte(RegNum, &bq24196_reg);
    pr_info("[bq24196_config_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg);

    bq24196_reg &= ~(MASK << SHIFT);
    bq24196_reg |= (val << SHIFT);

    ret = bq24196_write_byte(RegNum, bq24196_reg);
    pr_info("[bq24196_config_interface] Write Reg[%x]=0x%x\n", RegNum, bq24196_reg);

    // Check
    //bq24196_read_byte(RegNum, &bq24196_reg);
    //pr_info("[bq24196_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24196_reg);
...
void bq24196_dump_register(void)
{
    ...
    for (i=0;i<bq24196_REG_NUM;i++)
    {
        ...
        pr_notice("[0x%x]=0x%x ", i, bq24196_reg[i]);
    }
    pr_notice("\n");
}
...
static ssize_t store_bq24196_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        pr_notice("[store_bq24196_access] buf is %s and size is %zu \n",buf,size);
        ...
        else
        {
            ...
            pr_notice("[store_bq24196_access] Please use \"cat bq24196_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int __init bq24196_subsys_init(void)
{
    ...
    pr_notice("[bq24196_init] init start. ch=%d\n", BQ24196_BUSNUM);
