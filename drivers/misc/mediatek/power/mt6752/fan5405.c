...
kal_uint32 fan5405_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = fan5405_read_byte(RegNum, &fan5405_reg);
    battery_xlog_printk(BAT_LOG_CRTI,"[fan5405_config_interface] Reg[%x]=0x%x\n", RegNum, fan5405_reg);
    ...
    battery_xlog_printk(BAT_LOG_CRTI,"[fan5405_config_interface] write Reg[%x]=0x%x\n", RegNum, fan5405_reg);
...
/* Vanzo:zhangxinyu on: Wed, 8 Apr 2015 11:16:57 +0800
 * add fan5405 safety_limit attr
 */

kal_uint8 s_reg_value_fan5405=0;
static ssize_t show_fan5405_safety_limit_reg(struct device *dev,struct device_attribute *attr, char *buf)
{
    fan5405_read_byte(6, &s_reg_value_fan5405);
    battery_xlog_printk(BAT_LOG_FULL,"[show_fan5405_safety_limit_reg] 0x%x\n", s_reg_value_fan5405);
 ...
 */
static DEVICE_ATTR(fan5405_access, 0664, show_fan5405_access, store_fan5405_access); //664
static DEVICE_ATTR(fan5405_safety_limit_reg, 0664, show_fan5405_safety_limit_reg, NULL); //664
static struct device_attribute * fan5405_attr_list[] =
{
  &dev_attr_fan5405_access,
  &dev_attr_fan5405_safety_limit_reg,
};

static int fan5405_user_space_probe(struct platform_device *dev)
{
  int idx = 0, err = 0;
  int num = (int)(sizeof(fan5405_attr_list)/sizeof(fan5405_attr_list[0]));

  battery_xlog_printk(BAT_LOG_CRTI,"******** fan5405_user_space_probe!! ********\n" );

  for(idx = 0; idx < num; idx++)
  {
    if(err = device_create_file(&(dev->dev), fan5405_attr_list[idx]))
    {
      printk("device_create_file (idx %d) = %d\n", idx, err);
      break;
