...
static void mali_l2_cache_core_set_counter_internal(struct mali_l2_cache_core *cache, u32 source_id, u32 counter)
{
	...
	core_is_on = mali_l2_cache_lock_power_state(cache); //superdragonpt: TODO check this
...
void mali_l2_cache_reset(struct mali_l2_cache_core *cache)
{
	...
	/* Restart any performance counters (if enabled) */ //superdragonpt TODO but why restarting this?
	if (cache->counter_src0 != MALI_HW_CORE_NO_COUNTER) {
...
{
	MALI_DEBUG_ASSERT_POINTER(cache); //superdragonpt : find a way of disabling this in the future .. We don't want debug shit eating system resources .. newbs -.- 
