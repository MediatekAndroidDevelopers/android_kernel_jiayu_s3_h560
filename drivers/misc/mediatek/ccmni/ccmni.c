	...
	switch(cmd) {
		...
		case SIOCCCMNICFG:
			...
			if (md_id_irat == ccmni->md_id) {
				CCMNI_INF_MSG(md_id, "SIOCCCMNICFG: CCMNI%d iRAT on the same MD%d, usage_cnt=%d\n", \
			...
			CCMNI_INF_MSG(md_id, "SIOCCCMNICFG: CCMNI%d iRAT from MD%d to MD%d, usage_cnt=%d\n", \
...
static void ccmni_md_state_callback(int md_id, int rx_ch, MD_STATE state)
{
	...
	if ((state != TX_IRQ) && (atomic_read(&ccmni->usage)>0)) {
...
static void ccmni_dump(int md_id, int rx_ch, unsigned int flag)
{
	...
	CCMNI_INF_MSG(md_id, "CCMNI%d(%d), rx=(%ld,%ld), tx=(%ld, %ld), txq_len=%d, tx_busy=%ld\n", \
				ccmni->index, atomic_read(&ccmni->usage), ccmni->dev->stats.rx_packets, ccmni->dev->stats.rx_bytes, \
				ccmni->dev->stats.tx_packets, ccmni->dev->stats.tx_bytes, ccmni->dev->qdisc->q.qlen, ccmni->tx_busy_cnt);
