...
static int mmc3416x_init_flag =-1;
...
#ifdef CONFIG_COMPAT
static int mmc3416x_compat_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
    long ret =0;
	...
	switch (cmd)
	{
		case COMPAT_MMC31XX_IOC_TM:
             ...
             ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC31XX_IOC_TM,
                            (unsigned long)(arg32));
             if (ret){
       ...
       case COMPAT_MMC31XX_IOC_SET:
                 ...
                 ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC31XX_IOC_SET,
                                (unsigned long)(arg32));
                 if (ret){
                    MMCDBG("ECS_IOCTL_SET_MODE unlocked_ioctl failed.");
                    return ret;
       ...
       case COMPAT_MMC31XX_IOC_RESET:
                 ...
                 ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC31XX_IOC_RESET,
                                (unsigned long)(arg32));
                 if (ret){
                    MMCDBG("ECS_IOCTL_SET_MODE unlocked_ioctl failed.");
                    return ret;
       ...
       case COMPAT_MMC31XX_IOC_READ:
                 ...
                 ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC31XX_IOC_READ,
                                (unsigned long)(arg32));
                 if (ret){
                    MMCDBG("ECS_IOCTL_SET_MODE unlocked_ioctl failed.");
                    return ret;
       ...
       case COMPAT_MMC31XX_IOC_READXYZ:
                 ...
                 ret = file->f_op->unlocked_ioctl(file, COMPAT_MMC31XX_IOC_READXYZ,
                                (unsigned long)(arg32));
                 if (ret){
                    MMCDBG("ECS_IOCTL_SET_MODE unlocked_ioctl failed.");
                    return ret;
       ...
       case COMPAT_ECOMPASS_IOC_GET_DELAY:			
                 ...
                 ret = file->f_op->unlocked_ioctl(file, COMPAT_ECOMPASS_IOC_GET_DELAY,
                                (unsigned long)(arg32));
                 if (ret){
                    MMCDBG("ECS_IOCTL_SET_MODE unlocked_ioctl failed.");
                    return ret;
       ...
       case COMPAT_ECOMPASS_IOC_SET_YPR:			
                 ...
                 ret = file->f_op->unlocked_ioctl(file, COMPAT_ECOMPASS_IOC_SET_YPR,
                                (unsigned long)(arg32));
                 if (ret){
                    MMCDBG("ECS_IOCTL_SET_MODE unlocked_ioctl failed.");
                    return ret;
       ...
       case COMPAT_ECOMPASS_IOC_GET_OPEN_STATUS:
                 ...
                 ret = file->f_op->unlocked_ioctl(file, COMPAT_ECOMPASS_IOC_GET_OPEN_STATUS,
                                (unsigned long)(arg32));
                 if (ret){
                    MMCDBG("ECS_IOCTL_SET_MODE unlocked_ioctl failed.");
                    return ret;
...
static int mmc3416x_get_id(struct i2c_client *client)
{
     u8 msensor_id[2];
     u8 addr = 0x20;
     ...
     {
         printk(KERN_ERR "mmc3524x: read id = %d!\n",msensor_id[0]);
     }
    if(msensor_id[0] == MMC3416X_ID){
      return 0;
    }
    else   
      return -1;
...
static int mmc3416x_local_init(void) 
{
	...
	printk("mmc3416_local_init 1\n");
    if(mmc3416x_init_flag == -1)
    {		
	  MMCDBG("%s: FAIL\n", __func__);
      return -1;
    }
	printk("mmc3416_local_init 2\n");
	MMCDBG("%s: OK\n", __func__);
	return 0;
}
/*----------------------------------------------------------------------------*/
...
static int __init mmc3416x_init(void)
{
    ...
    printk("mmc3416_hwmsen_msensor_add start\n");
