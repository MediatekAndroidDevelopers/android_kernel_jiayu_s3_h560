...
void enc_isr(void)
{
    ...
    {
        MFV_LOGI("[enc_isr] VAL_DRIVER_TYPE_HEVC_ENC!!\n");
    }
    else
    {
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case VCODEC_SET_THREAD_ID:
            {
                MFV_LOGI("[K2] VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid);
                MFV_LOGI("[K2] VCODEC_SET_THREAD_ID [EMPTY] - tid = %d\n", current->pid);
            }
            break;
        ...
        case VCODEC_ALLOC_NON_CACHE_BUFFER:
            {
                MFV_LOGI("[K2][M4U]! VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n", current->pid);
        ...
        case VCODEC_FREE_NON_CACHE_BUFFER:
            {
                MFV_LOGI("[K2][M4U]! VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n", current->pid);
        ...
        case VCODEC_LOCKHW:
            {
                    ...
                    rHWLock.eDriverType == VAL_DRIVER_TYPE_VP8_DEC)
                {
                    while (bLockedHW == VAL_FALSE)
                    {
                        ...
                        {
                            MFV_LOGI("[NOT ERROR][VCODEC_LOCKHW] First Use Dec HW!!\n");
                            FirstUseDecHW = 1;
                        }
                        else
                        {
                        ...
                        {
                            MFV_LOGI("[NOT ERROR][VCODEC_LOCKHW] E\n");
                            ...
                            MFV_LOGI("[VCODEC_LOCKHW] Lock Instance = 0x%lx, Lock TID = %d, CurrentTID = %d, rLockedTime(%d s, %d us), rCurTime(%d s, %d us)\n",
                     ...
                     rHWLock.eDriverType == VAL_DRIVER_TYPE_JPEG_ENC)
                {
                    ...
                    // Wait to acquire Enc HW lock
                        ...
                        {
                            MFV_LOGI("[NOT ERROR][VCODEC_LOCKHW] First Use Enc HW %d!!\n", rHWLock.eDriverType);
                            FirstUseEncHW = 1;
                        }
                        else
                        {
                        ...
                        {
                            MFV_LOGI("Lock ok grVcodecEncHWLock.pvHandle = 0x%lx, va:%lx, type:%d", (VAL_ULONG_T)grVcodecEncHWLock.pvHandle, (VAL_ULONG_T)rHWLock.pvHandle, rHWLock.eDriverType);
                            gLockTimeOutCount = 0;
                        }
                        mutex_unlock(&VencHWLock);
                    }
        ...
        case VCODEC_WAITISR:
            ...
            {
                MFV_LOGI("[K2] VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
                MFV_LOGI("[K2] VCODEC_INITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
            }
            break;
            ...
            {
                MFV_LOGI("[K2] VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
                MFV_LOGI("[K2] VCODEC_DEINITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
            }
            break;
        ...
        case VCODEC_SET_CPU_OPP_LIMIT:
            {
                MFV_LOGI("[K2] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n", current->pid);
                user_data_addr = (VAL_UINT8_T *)arg;
                ...
                MFV_LOGI("-VCODEC_SET_CPU_OPP_LIMIT tid = %d, ret = %lu\n", current->pid, ret);
                MFV_LOGI("[K2] VCODEC_SET_CPU_OPP_LIMIT [EMPTY] - - tid = %d\n", current->pid);
            }
            break;
...
static int vcodec_open(struct inode *inode, struct file *file)
{
    ...
    MFV_LOGI("vcodec_open pid = %d, K2Driver_Open_Count %d\n", current->pid, K2Driver_Open_Count);
...
static int venc_hw_reset(int type)
{
    ...
    {//step 4
        ...
        VDO_HW_WRITE(KVA_VENC_SW_PAUSE, 0);
        MFV_LOGI("4 HRST = %d, isr = 0x%x", uValue, gu4HwVencIrqStatus);
    }

    VDO_HW_WRITE(KVA_VENC_SW_HRST_N, 1);
    uValue = VDO_HW_READ(KVA_VENC_SW_HRST_N);
    MFV_LOGI("HRST = %d", uValue);
...
static int vcodec_flush(struct file *file, fl_owner_t id)
{
    ...
    MFV_LOGI("vcodec_flush pid = %d, K2Driver_Open_Count %d\n", current->pid, K2Driver_Open_Count);
...
static int vcodec_release(struct inode *inode, struct file *file)
{
    ...
    mutex_lock(&DriverOpenCountLock);
    MFV_LOGI("vcodec_release pid = %d, K2Driver_Open_Count %d\n", current->pid, K2Driver_Open_Count);
    K2Driver_Open_Count--;
    ...
    if (K2Driver_Open_Count == 0)
    {
        ...
        if (gu4L2CCounter != 0)
        {
            MFV_LOGI("vcodec_flush pid = %d, L2 user = %d, force restore L2 settings\n", current->pid, gu4L2CCounter);
            if (config_L2(1))
            {
                MFV_LOGI("restore L2 settings failed\n");
            }
        }
        gu4L2CCounter = 0;
        mutex_unlock(&L2CLock);
...
static int vcodec_mmap(struct file* file, struct vm_area_struct* vma)
{
    ...
    MFV_LOGI("[mmap] vma->start 0x%lx, vma->end 0x%lx, vma->pgoff 0x%lx\n",
